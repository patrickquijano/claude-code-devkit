...
- **FR-001**: No script under `scripts/` MUST execute another script under `scripts/`.
- **FR-002**: Every entry point under `scripts/` MUST be a wrapper that resolves the repository root, sources one file from `scripts/lib/`, and calls it.
...
- **FR-009**: A usage header MUST NOT promise a `--fix` behaviour its check does not have.
...
- **SC-003**: `grep` for a `scripts/` path being executed from another `scripts/` file returns nothing.
