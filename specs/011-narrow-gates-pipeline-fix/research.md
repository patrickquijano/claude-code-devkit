...
**That prediction is not what happened, and the record says so rather than being quietly corrected.** Two files were compacted by hand and measured in characters: `tooling.md` reached **8%** and `reference/verify.md` **6%**, neither clearing the floor. The prediction was that size predicts compressibility; the measurement says density does, and these documents are already dense because this rule set forbids padding and requires a recorded rationale beside anything surprising, so nearly every line carries a backticked identifier, a number or a `because`. Whether 15% is therefore the wrong number was left open at that point, deliberately, rather than resolved by lowering the floor to whatever the two samples happened to reach.

**Resolved: the floor stays at 15%, and FR-027 is withdrawn instead.** The maintainer settled this after the unit fix landed. The reasoning is that a threshold nothing in a corpus reaches is evidence about the corpus, not about the threshold. Two readings were available and only one survives contact with what 15% is for: either the bar is too high, or these documents are not padded. The second is what the measurements say — both compactions dropped nothing normative and still could not find 15% of removable text, which is the signature of prose already at its information floor, not of an unreasonable bar.

Lowering it to 8% would have been fitting the rule to two data points, and would have destroyed what the number does. 15% exists so that a change cannot be called a compaction on the strength of deleting a few words; a floor set to whatever the last attempt achieved can never fail, and a criterion that cannot fail is the status quo CHK011 flagged, wearing a number.

So the mechanism survives its own requirement's withdrawal, and that is the intended outcome rather than a consolation. The carve-out in `.claude/rules/repository-docs.md`, this definition, R1's normative-content rule, R3's audit script and its five fixtures are all in place and all exercised end to end on one document. A future contributor facing a genuinely padded file runs `sh scripts/compaction-audit.sh <ref> <path>` and gets an answer. What was withdrawn is the obligation to go looking for such files here, having looked and not found them.

Recorded in `spec.md` under Withdrawn within this feature, and in `tasks.md` where T058–T074 and T081–T083 are marked won't-do.
