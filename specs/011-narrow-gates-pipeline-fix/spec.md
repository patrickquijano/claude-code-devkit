...
- **FR-001**: The eight-phase workflow MUST request an approval before a phase when that phase's proposal differs in any way from what the maintainer approved in the plan. **"The plan" is the set of all eight phase arguments the workflow drafts and displays together before the first phase runs, per FR-006.** That set is the one side of the comparison; the proposal about to run is the other. A difference of any kind — reworded but equivalent, reordered, or a differing list of artifacts — is a difference, and asks.
- **FR-002**: The eight-phase workflow MUST request an approval before any step whose effect is not readily undone, and MUST treat implementing, committing, pushing, raising a review request, deleting a branch, and creating or removing a workspace as such steps. **An effect is "readily undone" when the maintainer can reverse it alone, in the working tree, without recovering deleted data and without an action by any remote service or other person.** Writing a specification artifact is readily undone; pushing, opening a review request and deleting a branch are not, because reversing each requires the forge. This definition governs; the enumerated list is what it yields for the steps known today, and is not exhaustive.
...
- **FR-007**: A maintainer MUST be able to require an approval before every phase for a run, overriding FR-003. **The choice is offered once, at the point the plan is presented and before the first phase runs, and it holds for the whole run.** It is not offered again at each approval — a question repeated at every boundary is the click-through this feature exists to remove — and it is recorded rather than remembered, so a run resumed after an interruption applies the mode the maintainer chose rather than the default. [CHK027]
...
- **FR-009**: The toolkit MUST offer a capability that takes a maintainer from a failed continuous-integration pipeline to a verified fix. **A fix is "verified" when the bug-remediation workflow's own validation stage has run against it and recorded a result of verified** — not when the code change merely exists, and not when validation was skipped. A validation result of partial or failed is not a verified fix, and FR-016 governs what happens then.
- **FR-010**: The capability MUST support pipelines on both forges the toolkit already supports — **GitHub and GitLab** — and MUST report a plain, non-failing skip for a repository hosted elsewhere, having no remote, or lacking the means to read its pipeline. The two are named here so this requirement's scope can be read without consulting the codebase.
...
- **FR-013**: The capability MUST offer the maintainer **at least two** stated alternative remediation approaches and MUST obtain their choice before anything is changed. A single approach is not an offer of alternatives. Where the capability can genuinely identify only one, it MUST say so rather than presenting one option as a choice.
...
- **FR-018**: Where the evidence indicates a cause outside the repository's own content, the capability MUST be able to report that as its finding rather than proceeding to a code change. **The boundary is whether a change to a file in this repository would fix it.** A provider outage, an expired credential and a runner fault are outside; a dependency the repository pins is **inside**, because the pin is a file here and changing it is a fix — even though the breakage originated upstream. Where the boundary is genuinely unclear, the capability MUST put it to the maintainer rather than deciding.
- **FR-019**: Where the fix would add or alter a requirement, the capability MUST report that the work is feature work rather than defect work, and MUST NOT carry it through the defect path. **The maintainer decides, and the decision is put to them at the point the remediation approach is chosen** — FR-013's boundary, after a root cause is agreed and before anything is changed. The capability raises the question with its reasoning; it MUST NOT classify the work as feature work on its own and stop.

#### How questions are asked

- **FR-020**: Every point at which any of the toolkit's workflows requires a decision from the maintainer MUST present the available choices. A question posed as prose alone MUST NOT be used. **The steps FR-002 names are a subset of the points this requirement governs**: FR-002 decides _whether_ those steps ask, and FR-020 through FR-023 decide what the question looks like once asked. The two do not conflict and neither overrides the other. One exception is stated at FR-036.
...
- **FR-023**: Where recommending a choice would be inappropriate because the decision is the maintainer's alone and no default is defensible, the question MUST say so explicitly rather than omitting the recommendation without comment. **The criterion is evidential, not a matter of preference: no default is defensible only when the workflow holds no fact that distinguishes the options.** Where it holds such a fact — a measurement, a recorded rule, a cost one option carries and another does not — a recommendation is defensible and MUST be given, however uncomfortable. Difficulty in choosing is not the criterion; absence of evidence is.
...
- **FR-027a**: The artifacts the workflows write to disk are out of scope for shortening. They MUST NOT be compacted by this feature. **Unaffected by FR-027's withdrawal** — it excluded a category from a scope that no longer applies, and it stays as the record of that exclusion. The tooling the exclusion rests on is **Spec Kit's `token-budget` extension**, registered in `.specify/extensions.yml` and offering `speckit.token-budget.compact` after the tasks phase; it is what already compacts written artifacts, which is why compacting them here would duplicate rather than add.
...
- **FR-031**: Shortening MUST NOT breach the length, structure and formatting limits the repository already places on these documents. **Those limits are recorded in two files and nowhere else**: `.claude/rules/skill-authoring.md` § The three budgets — `SKILL.md` under 500 lines, `description` plus `when_to_use` under 1,536 characters, the first 5,000 tokens surviving compaction — and `.claude/rules/repository-docs.md` § `CLAUDE.md` and § Markdown mechanics — `CLAUDE.md` under 200 lines, no hard-wrapped prose, two-space indentation inside fences.
...
- **FR-032**: The requirements of feature `006-claude-code-guidance` that mandate an approval before every phase MUST be recorded as superseded by FR-001 through FR-005, with feature 006's own specification left intact as the record of what it shipped. **This obligation extends to 006 FR-013, which is not an approval requirement**: it governs what a proposal states — that it names what changed since the plan — and it is superseded by FR-001 and FR-004 rather than by FR-001 through FR-005. Naming it separately is what stops a superseded requirement going unaccounted for because it did not fit the category the rest fell into.
...
- **FR-034**: The recorded reasoning that ties the removal of per-phase approvals to a change in how the workflow may be invoked MUST be re-examined and its outcome recorded, since this feature narrows those approvals rather than removing them. **The record MUST state which of two outcomes obtains — the reasoning survives, or it does not — and MUST name the property it depends on and say whether that property still holds.** A record that asserts re-examination happened without naming that property does not satisfy this requirement.
...
#### What this feature cannot verify about itself

- **FR-037**: This repository runs no continuous-integration pipeline of its own, so the pipeline capability's forge behaviour cannot be exercised here. **Wherever this feature's verification is reported, that limit MUST be stated as a limit rather than the affected scenarios being reported as passing or omitted.** The assumption that the existing bug-remediation workflow is fit to receive pipeline defects unchanged MUST be recorded as **unvalidated** until a run against a repository with a real failing pipeline validates it. An assumption that a feature depends on and cannot test is a risk, and it is recorded as one rather than as a premise.

#### Gaps the requirements-quality review found

These requirements close items the review recorded at `checklists/quality.md`. Each names the item that produced it. They add obligations to the workflows rather than restating existing ones.

- **FR-038**: A completed run MUST record, per phase, which of FR-001 and FR-002 caused an approval to be requested, or that neither applied. Announcing the reason in the run's output is not sufficient: SC-002 is verified by inspecting the record after the run, and output is not a record. [CHK015]
- **FR-039**: Where FR-016's return to diagnosis is taken, the second and every later cycle MUST re-obtain FR-012's approval of the root cause and FR-013's choice of approach. A cycle MUST NOT reuse an earlier cycle's answers, because the evidence that produced them is what the return exists to revisit. [CHK023]
- **FR-040**: Where more than one job in a run has failed, the capability MUST offer addressing them together as well as choosing one. FR-017 forbids choosing silently; this requires that "all of them" be among the choices, since several jobs failing on one cause is the common case rather than the exceptional one. [CHK030]
- **FR-041**: Where retrieval succeeds but returns evidence that is empty, truncated by the forge, or expired by its retention policy, the capability MUST report which of those occurred and MUST fall back to FR-011b's request for supplied output. This case is neither FR-011a's success nor FR-011b's failure, and without this requirement it falls between them. [CHK031]
- **FR-042**: Where the failing run's evidence does not establish a cause — the failure is intermittent, or the same job passes on retry — the capability MUST report that the evidence is inconclusive and MUST offer the maintainer gathering evidence from further runs rather than proposing a cause the evidence does not support. [CHK032]
- **FR-043**: Where FR-030 leaves a document unshortened, the reason and the percentage actually reached MUST be recorded durably in the feature's own artifacts, not only reported in the run's output. A reason stated once in passing is not available to the reviewer who reads the change later. [CHK034]
- **FR-044**: Where a phase that proceeded without an approval under FR-003 then fails, the workflow MUST state that the failed phase was one nobody approved individually. It MUST NOT present the failure as though an approval had covered it, and it MUST NOT treat the auto-proceed as a reason to continue past the failure. [CHK035]
- **FR-045**: Shortening MUST preserve a passage that is not normative under FR-028's definition but records why an odd-looking rule exists. Such a passage is load-bearing: losing it invites a later contributor to "fix" the rule it explains. Where the operational definition of normative content does not already capture such passages, that definition MUST be widened rather than the passage removed. [CHK036]
- **FR-046**: Where the maintainer disputes that the retrieved run is the one they meant, the capability MUST let them name a different run and MUST re-retrieve rather than proceeding on the disputed evidence or requiring the run be restarted. FR-017 covers ambiguity the capability can see; this covers a retrieval that looked unambiguous and was wrong. [CHK037]
- **FR-047**: Where forge access is present at FR-011c's determination but absent or failing when retrieval is attempted, the capability MUST treat it as FR-011b's case and say that access changed during the run. Reporting only "retrieval failed" leaves the maintainer debugging a configuration that was working minutes earlier. [CHK041]
- **FR-048**: Where a run is interrupted between an approval and the step it approved, the approval MUST NOT be treated as still held when the run resumes. The step MUST be proposed again. An approval is given against a state of the world that an interruption may have changed, and FR-005's guarantee is only as good as this. [CHK042]
- **FR-049**: Where the bug-remediation workflow the capability invokes stops or fails rather than completing, the capability MUST report that plainly, MUST NOT report a verified fix, and MUST NOT carry out that workflow's remaining stages itself. FR-016 covers validation not showing the defect resolved; this covers the workflow not reaching validation at all. [CHK043]
...
- **SC-001**: On a run where nothing is revised after the plan, the eight-phase workflow requests approvals at **exactly the boundaries the always-gate set names and no others** — six of them, per `contracts/gate-decision.md`. Counting fewer approvals than phases is necessary but not sufficient: seven of eight would satisfy a bare inequality and would mean the narrowing had barely happened.
...
- **SC-009**: The standard governing questions is stated in exactly one place. Zero restatements. **A restatement is a passage that would still tell a reader what the standard requires if the single home were deleted; a pointer is one that would not.** Naming the rule, linking to it, or saying that questions follow it are pointers and are permitted without limit. Reproducing any of its obligations — options, per-option cost, one recommendation, the reason — is a restatement, whatever wording it uses.
- **SC-010**: Every shortened document **clears the length floor `research.md` R2 sets — at least 15% of its non-code, non-frontmatter, non-blank characters — or is recorded exempt under FR-030 with the percentage it actually reached**, and retains every normative statement from its previous version. Zero normative statements lost. Stating the floor here rather than only in R2 is what stops this criterion being satisfied by removing one character. **Met over a denominator of one.** With FR-027 withdrawn, exactly one document was shortened — `skills/ccd-speckit-run/reference/tooling.md`, 8%, `normative-lost 0` — so "every shortened document" quantifies over a set of one and reads stronger than the evidence behind it. The denominator is stated here rather than left to be discovered, because a criterion that is vacuously or nearly-vacuously true is the kind a later reader cites as though it covered the corpus.
...
- **SC-013**: Every failure path the pipeline capability can meet is named in its instructions and produces a stated outcome — no CLI, not authenticated, retrieval failed, evidence empty or truncated or expired, evidence inconclusive, the wrong run retrieved, access lost mid-run, the bug workflow stopping. Zero paths end in silence or in an unqualified claim of a verified fix.
- **SC-014**: A completed eight-phase run's record answers, for every phase, why it did or did not ask — readable after the run without the run's output. Zero phases unaccounted for in the record.
- **SC-015**: No approval survives an interruption. A run resumed after an interruption proposes again every step whose approval was given before it. Zero steps executed on an approval given before an interruption.
