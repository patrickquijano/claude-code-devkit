...
> **FR-013, FR-013a, FR-013b and FR-013c are superseded** by [`specs/004-format-hook-scope/contracts/exclusion-declaration.md`](../004-format-hook-scope/contracts/exclusion-declaration.md), which replaced one central list plus five mirrors with one declaration per check, and deleted both `.lintignore` and the `scope` check that policed the agreement. FR-013's substance survives — the same paths are still excluded, proved unchanged path-for-path — but the mechanism the four requirements describe no longer exists. FR-013b in particular required a means of verifying agreement between copies; with one declaration per check there are no copies to disagree. The four below are a true record of what shipped under this feature and are no longer current.
