...
**Amendment, 2026-09-02 (second).** One change, and it is the first in this feature that produces an artifact for people rather than for tools. The repository gains three change-proposal templates: one general structure applied to every proposal, and two selected by URL — one for a change to the checking machinery, one for a change to the spec-driven record. They exist because the constitution already obliges an author to have run and passed the aggregate check before proposing (`constitution.md:167`) and obliges a reviewer to verify principle compliance (`:189`), and until now nothing carried either obligation to either person.

The design turns on one platform fact and one platform limit. The fact: only a single template file is applied automatically, so a directory of templates alone yields an empty proposal body — everything required lives in the general structure, which is what FR-033 encodes. The limit: issue forms are not supported for pull requests, so there is no required field and no validation, and the structures are advisory in enforcement and mandatory in content. A third piece follows from the repository's own standards rather than from the platform: these are Markdown files that nothing excludes, so they enter lint scope automatically and had to be **designed to pass** the checks — an eighth check, `scripts/lint-citations.sh`, then verifies that every governance quotation in them still matches the document it cites.
...
**Testing**: the checks are the tests. `scripts/lint.sh` from a clean checkout is the verification command. One deliberately non-conforming fixture per check proves the check actually fails, run through a self-test script rather than committed into the tree in a state that would fail the aggregate run. The amendment adds fixtures for the two content kinds the formatter newly covers, and adds `scripts/lint-scope.sh` — the scope-agreement verification FR-013b requires, which is a check in its own right and therefore part of the aggregate. The second amendment adds `scripts/lint-citations.sh` on the same terms, plus a fixture for it: a template whose governance quotation has been altered must be rejected and named. What is deliberately **not** self-tested is whether the template prose is clear or useful — that has no decidable failure condition, and claiming a test for it would be claiming a verification that did not happen (research.md §23).
...
**Scale/Scope**: 7 configuration files (`.editorconfig-checker.json` is new), 13 shell files (10 runners plus `selftest.sh` and 3 in `lib/`, once `lint-scope.sh` and `lint-citations.sh` exist), 2 plugin manifests, 1 scope declaration, 1 gitignore, **3 change-proposal templates**, 1 extensions manifest with 6 installed extensions and **28** registered hooks (git 18, token-budget 6, agent-context 2, superb 2, assess 0, bug 0) -- counted from the installed manifest, not from the catalog, which advertises 5 hooks for `superb` where v1.9.0 registers 2. The aggregate check runs **8** checks. Repository holds 169 tracked paths at `701f290`.
...
**The two obligations this amendment gives an artifact to.** Stated explicitly because they are the reason the second amendment exists, and because a design that merely mentions them would satisfy nothing. Development Workflow: "Before a change is proposed for review, the aggregate quality check MUST have been run and MUST have passed." The general structure asks the author to state whether it was, quoting that sentence at its source. Governance: "All reviews MUST verify compliance with these principles." The general structure's reviewer section names the six principles, quoting that sentence likewise. Neither is enforced — the platform offers no required field — and `scripts/lint-citations.sh` verifies that both quotations still match the constitution's current wording, which is the part that _can_ be enforced.

**Why the citation check is not a second linting configuration for Markdown**, which is the first thing a reviewer should challenge against the Quality Gate Requirements clause. Markdown already has one configuration per concern: `.prettierrc.json` formats it and `.markdownlint-cli2.jsonc` lints it. `lint-citations.sh` adds neither — it has **no configuration file of its own** and governs no content kind, verifying an invariant between two files instead. `scripts/lint-scope.sh` set that precedent in the first amendment on identical terms. The clause counts configurations per content kind and concern; this check contributes zero of them.

**Post-design re-check**: passes against v1.2.0. The amendment adds two pinned plugins, one configuration file, two scripts and one extensions manifest — each named by a requirement. Two new costs are recorded in Complexity Tracking rather than waved through: scope agreement is now verified rather than structural, and ShellCheck has no ignore mechanism to declare. The second amendment adds one more — an eighth check and a marker convention inside a prose artifact — and one design change forced by this repository's own rules: MD041 requires every template to open with a level-one heading, which sits under the proposal's own title field. The alternative was excluding `.github/` from the Markdown check, which FR-037 forbids.
...
│   └── proposal-review.md # requirements-quality review of the second amendment
...
```text
.github/
├── pull_request_template.md     # the general structure; the only one applied automatically
└── PULL_REQUEST_TEMPLATE/       # opt-in, selected by ?template=<filename>.md
    ├── quality-gate.md          # a change to the checking machinery
    └── spec-record.md           # a change to the spec-driven record
...
scripts/
├── lint.sh                      # aggregate entry point; runs all eight, stops at first failure
...
├── lint-citations.sh            # FR-036: every governance quotation matches the document it cites
...
### Design detail: the three templates and what each one asks

`.github/pull_request_template.md` is the only file GitHub applies without being asked, so it carries
everything required. Its sections, in order:

1. **A level-one heading.** Forced by MD041 and kept because it names which of the three structures the
   author is looking at, which is otherwise invisible once the content is in the proposal body.
2. **What changed** and **Why** — FR-029. The second is the one reviewers cannot recover from the diff.
3. **Where to look first.** Adopted from GitHub's own guidance, which asks for "where reviewers should
   pay special attention" alongside what and why.
4. **Checks** — FR-030. One statement about `scripts/lint.sh`, with the Development Workflow sentence
   quoted beneath it under a `<!-- cite: -->` marker.
5. **How this was produced** — FR-034. Whether a coding agent produced the change, and if so a
   reference to the session.
6. **For the reviewer** — FR-031 and FR-035, in one section rather than two. The six principles named,
   with the Governance sentence quoted under its own marker.

`.github/PULL_REQUEST_TEMPLATE/quality-gate.md` adds the question a change to the checking machinery
raises and nothing else does: how the change was proven to leave every check still able to fail on bad
input. That is `scripts/selftest.sh`, and the specialised structure asks for its result rather than
assuming it.

`.github/PULL_REQUEST_TEMPLATE/spec-record.md` adds the two questions a change to the spec-driven record
raises: which phase produced it, and whether the existing requirement and criterion numbering was
continued rather than restarted. Both are mistakes this feature's own history has already made once.

Each specialised file **repeats** the general structure's sections rather than referring to them. That
looks like duplication and is required: selecting a specialised template replaces the body entirely, so
a section that exists only in the general file is a section the author never sees. FR-033 is what makes
this explicit, and SC-019 counts it.

### Design detail: the citation marker, and why the checker normalises whitespace

A citation is an HTML comment naming the cited file, followed immediately by a blockquote:
...
`scripts/lint-citations.sh` finds each marker, joins the blockquote that follows it to a single line,
strips the `>` prefixes, and requires the result to appear in the cited file. Exit `0` if every
citation matches, `1` naming each that does not. No `--fix`: which of the two texts is wrong is a
judgement, not a rewrite.

The whitespace normalisation is the load-bearing part, and it was measured rather than assumed. The
constitution hard-wraps its prose, so the sentence FR-030 quotes spans `constitution.md:167` and `:168`.
A naive `grep -F` for the one-line quotation returns **zero** matches — the check would reject a
citation that is perfectly accurate. Flattening the cited file with `tr` and collapsing runs of spaces
makes both target sentences match, and the same sentence with `MUST` changed to `SHOULD` correctly does
not. research.md §23 records all three results.

The marker is an HTML comment rather than a fenced block or a custom syntax for two reasons: Prettier
leaves it untouched, verified against the committed configuration, and it disappears when GitHub renders
the proposal body, so the author never sees plumbing.
...
| **An eighth check, and a marker convention embedded in a prose artifact.** `scripts/lint-citations.sh` plus a `<!-- cite: -->` comment in every template that quotes governance.                                                                                                                                                | FR-036 requires a stale quotation to be detectable by running something. The Phase 4 checklist flagged the requirement as unverifiable while it said only that, so the plan had to name a mechanism or record why it could not. Without a marker there is nothing to parse: a blockquote in a Markdown file is not distinguishable from an ordinary quotation. | Trusting review to notice a stale quotation was the status quo and is what FR-036 was written against — the constitution has already been amended twice during this feature, so the quotations will go stale. A markdownlint custom rule was rejected because it resolves through Node module lookup and would work on some machines and not others, the same reasoning that kept the Prettier preset inline in research.md §16. Folding the comparison into `lint-scope.sh` was rejected as two unrelated checks under one name. The residual cost: three template files now contain a convention a reader has to be told about, and the plan is where they are told. |
| **Each specialised template repeats the general one's sections verbatim.** Three files carrying the same six headings, two of them carrying the same two governance quotations.                                                                                                                                                 | Selecting a specialised template replaces the proposal body entirely, so anything present only in the general file is invisible to an author who selected a specialised one. FR-033 forbids an obligation living only in an opt-in structure, and SC-019 counts it at zero.                                                                                    | Referring to the general structure instead of repeating it — "see the standard template" — was rejected: an author reading a body that says that has to go and find it, which is the friction the templates exist to remove, and the two governance quotations would then reach only the authors who did. Dropping the specialised templates entirely was offered to the user and declined; it would take the two questions they exist to ask with them. The residual cost is real: three files now have to be kept in step by hand, and `lint-citations.sh` catches only the quotations going stale, not the sections diverging.                                      |
| Two extensions are installed from a catalog whose discovery-only flag is stated to be the vetting boundary.                                                                                                                                                                                                                     | Requested explicitly, and reaffirmed after the boundary, the read-write effect, and the 12 commands and 8 hooks were shown. FR-026 requires the provenance be recorded, which research.md §17 does including the weaker pin on the tag tarball.                                                                                                                | Installing only the four vetted extensions was recommended and declined. Nothing was found to make the two safe rather than merely recorded; what is recorded is the exact URL, the version, the effect, and that a re-tagged release would change the bytes at the same URL for `token-budget`.                                                                                                                                                                                                                                                                                                                                                                       |
