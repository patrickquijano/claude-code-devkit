...
`claude-code-devkit` is a Claude Code plugin and developer toolkit for building custom agents, commands, skills, and MCP servers. It is early: at the time of writing `README.md` is the only tracked file, so most of the plugin surface described below is still being built rather than read from disk.
...
```sh
scripts/lint.sh       # all six checks; exits non-zero at the first failure
scripts/lint.sh --fix # rewrite what can be rewritten
scripts/selftest.sh   # prove each check still rejects bad input
```

Run one check in isolation with `scripts/lint-<standard>.sh`, where `<standard>` is one of `editorconfig`, `format`, `markdown`, `yaml`, `shell`, `python`.
