...
- **MUST NOT commit to or push `main`.** It is a merge target, not a working branch: commit and push from a feature branch and let a review land the change. Not overrideable.
...
- **The shell check's glob list carries two extensionless paths, and they are not redundant.** `scripts/lint-shell.sh` collects `'*.sh' '.husky/commit-msg' '.husky/pre-push'`. Git names a hook by its filename, so a hook file cannot end in `.sh`, and `'*.sh'` alone skips both dispatchers silently while still reporting success. Removing them stops checking the hooks. The hook logic itself lives in `scripts/lib/commit-msg.sh` and `scripts/lib/push-check.sh`, behind thin wrappers in `scripts/hooks/`, so the ordinary glob finds it and `selftest.sh` can call it with a fixture without executing a sibling script; details in [`.claude/rules/husky-git-hooks.md`](.claude/rules/husky-git-hooks.md) and [`docs/husky-git-hooks.md`](docs/husky-git-hooks.md).
