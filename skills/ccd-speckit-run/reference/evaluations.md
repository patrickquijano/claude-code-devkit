...
- Phase 1 classifies the constitution **absent**, reports the drafted principles, invokes the phase, and reports the Sync Impact Report. It does **not** ask before writing — Step 3 approved that prompt.
- Phases 1–8 run start to finish with no `AskUserQuestion` between them. Every phase still reports its artifacts and writes `phases.N`, and Phase 8 is preceded by a report of `analyze`'s findings and any surviving markers. A run that stops to ask "proceed to the next phase?" fails this case.
- **Silence is the default, not the rule.** This scenario has nothing that needs attention, so nothing stops. It does not assert that phases never stop — a failed phase, a conflict, or anything else meeting the attention test still asks, and doing so is correct behavior here, not a regression. E2 covers the conflict case; the failure case is E1's own variant below.
...
- No phase advances while the conflict is unresolved. This question survives the removal of the per-phase gates: phases run unattended, conflicts do not. A run that picks a resolution itself, or defers the conflict to Step 5, fails this case.
...
- Up to four sweeps dispatched in **one** batch, not four sequential dispatches: prior art, governance, existing artifacts, conventions.
