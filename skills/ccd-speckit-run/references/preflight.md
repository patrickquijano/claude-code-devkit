...
- State file absent but a spec directory exists → a run happened without this skill, or the file was deleted. Report the artifact inventory and the script's suggested resume point, then `AskUserQuestion` with the same four options as above. Recommend resuming from the script's suggested point, and say why: the artifacts on disk are the work, and restarting rewrites them from a spec that may already have been clarified.
